...
import com.todomanager.presentation.controller.TasksController;
import com.todomanager.presentation.controller.advice.ApiExceptionHandler;
...
class TasksControllerTest {
  ...
  void returnsTasksWithExpectedFields() throws Exception {
    ...
    given(tasksApplicationService.getTasks(1L)).willReturn(List
        .of(new TaskResponse(1L, "Buy milk", false), new TaskResponse(2L, "Write tests", true)));

    mockMvc.perform(get("/api/v1/tasks").header("X-User-Id", "1")).andExpect(status().isOk())
        .andExpect(jsonPath("$[0].id").value(1)).andExpect(jsonPath("$[0].title").value("Buy milk"))
        .andExpect(jsonPath("$[0].completed").value(false)).andExpect(jsonPath("$[1].id").value(2))
  ...
  void updatesTaskAndReturnsUpdatedResponse() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/tasks/2").header("X-User-Id", "1").contentType(MediaType.APPLICATION_JSON)
  ...
  void returnsBadRequestWhenHeaderIsMissing() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/tasks")).andExpect(status().isBadRequest())
  ...
  void returnsNotFoundWhenTaskDoesNotExist() throws Exception {
    MockMvc mockMvc = buildMockMvc();
    willThrow(new TaskNotFoundException(99L)).given(tasksApplicationService).updateTask(1L, 99L,
        new TaskUpdateRequest("missing", false));

    mockMvc
        .perform(
            put("/api/v1/tasks/99").header("X-User-Id", "1").contentType(MediaType.APPLICATION_JSON)
  ...
  void returnsForbiddenWhenTaskBelongsToAnotherUser() throws Exception {
    MockMvc mockMvc = buildMockMvc();
    willThrow(new TaskOwnershipException(2L, 1L)).given(tasksApplicationService).deleteTask(1L, 2L);
